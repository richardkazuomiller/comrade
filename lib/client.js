  ...
  this.googleDataset.runQuery(query,function(err,entities,endCursor){
    ...
    queue.on('end',function(){
      retval = retval.sort(function(a,b){
        return a.role.localeCompare(b.role) || a.created - b.created
      })
      callback(err,retval)
    })
    queue.start()
...
Client.prototype.killOldestMember = function(options,callback){
  var self = this
  if(!options.role){
    callback(new Error('You must set a role'))
    return;
  }
  self.getMembers(options,function(err,servers){
    ...
    var roleConfig = self.roles[options.role] || {}
    var minMembers = roleConfig.minMembers
    if(typeof minMembers != 'number'){
      minMembers = 1
    }
    if(oldestServer && livingCount > minMembers){
...
Client.prototype.rollingKill = function(options,callback){
  var self = this
  if(!options.role){
    callback(new Error('You must set a role'))
    return;
  ...
  self.getMembers(options,function(err,servers){
    ...
    servers.forEach(function(server){
      if(!server.shutdownDate){
        serversToKill.push(server.id)
        console.log('Going to shut down server '+server.id)
      }
    })
    console.log('-----')
    ...
    if(serversToKill.length <= minMembers){
      console.log('Waiting for more servers to come online')
      setTimeout(function(){
        self.rollingKill(options,callback)
      },10000)
      return;
    }
    ...
    function attempt(){
      self.getMembers(options,function(err,servers){
        var finished = true
        serversToKill.forEach(function(id){
          var isAlive = false
          servers.forEach(function(server){
            if(server.id == id && !server.shutdownDate){
              console.log('Server '+id+' is still alive')
              isAlive = true
            }
          })
          if(isAlive){
        ...
        if(finished){
          ...
          self.killOldestMember(options,function(){
